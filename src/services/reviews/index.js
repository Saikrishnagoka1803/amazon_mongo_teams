...
reviewsRouter.route("/:productId/reviews").post(async (req, res, next) => {
  try {
    const targetProduct = await Products.findById(req.params.productId);
    if (targetProduct) {
      const newComment = await Products.findByIdAndUpdate(
...
reviewsRouter
  ...
  .put(async (req, res, next) => {
    try {
      const targetProduct = await Products.findById(req.params.blogPostId);
      if (targetProduct) {
